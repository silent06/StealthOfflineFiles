...
BOOL InitializeSystemHooks() {
	...
	*(DWORD*)0x816FEE9C = 0x38600000;
	remove("HDD:\Cache\TP.0000000000000");
	remove("HDD:\\SysCache0:\TP.0000000000000");
	remove("HDD:\\SysCache1:\TP.0000000000000");
	INI::Init(); INI::Read();
	if (!FileExists("HDD:\\silent Cache\\RGB.ini")) WriteINI();
	else ReadINI();
	...
	SpoofedExecutionId.Version = ver;
	SpoofedExecutionId.BaseVersion = ver; 
	...
	if (PatchModuleImport(MODULE_XAM, MODULE_KERNEL, 404, (DWORD)XenonPrivilegeHook) != S_OK) return FALSE;
	...
	if(INI::CustomUI) tools.HookFunctionStart((DWORD*)0x817BE1C8, (DWORD*)HUD_UI::XuiElementBeginRenderStub, (DWORD)HUD_UI::XuiElementBeginRenderHook);
	return TRUE;
}